...
export const LiveStream = () => {
    ...
    const connectWebSocket = useCallback((streamId: string) => {
        const wsUrl = `ws://localhost:6996/ws/stream/${streamId}`;
    ...
    const handleStartStream = async () => {
        ...
        try {
            const response = await fetch('http://localhost:6996/api/stream/start', {
                method: 'POST',
    ...
    const handleStopStream = async () => {
        ...
        try {
            await fetch('http://localhost:6996/api/stream/stop', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
